# LEWD PREFETCH [![License](https://licensebuttons.net/l/by-nc-sa/4.0/88x31.png)](https://creativecommons.org/licenses/by-nc-sa/4.0/)

---

Script used to selectively prefetch world or HUD models into the memory.

---

### ABOUT

This script allows to prefetch world or HUD models into the memory.

Example :
```INI
![wpn_akm_alfa]
lewd_prefetch_world     = exists
lewd_prefetch_hud       = nearby
```

Valid modes are :
- `always` - always prefetch the model
- `exists` - prefetch the model if an object exists in the savegame with the given section
- `nearby` - same as `exist`, but the object has to be on the same map as the player
...
For past updates, please refer to the description of each release, in the [releases tab](https://github.com/nltp-ashes/LEWD-Prefetch/releases).
